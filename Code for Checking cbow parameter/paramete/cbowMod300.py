...
for i in range(10):
    ...
    forStore_m.append(storing)
for i in range(10):
    storing = open("context_"+str(i+1)+".txt", "w")
    forStore_c.append(storing)
for i in range(10):
    storing = open("num_workers_"+str(i+1)+".txt", "w")
    forStore_n.append(storing)
for i in range(4):
    storing = open("downsampling_"+str(i+1)+".txt", "w")
    forStore_d.append(storing)
...
while True:
    ...
    # print(string)
    forStore_m[min_word_count-1].write(string)
...
for i in range(10):
    ...
    for r in result_list:
        if r[2] == i+1:
            cnt += 1
            string = str(cnt) + ", " + str(r[0]) + ", " + str(r[1]) + ", " + str(r[3]) + ", " + str(r[4]) + ", " + str(r[5]) + "\n"
            forStore_c[i].write(string)
...
for i in range(4):
    ...
    for r in result_list:
        if r[3] == ds[i]:
            cnt += 1
            string = str(cnt) + ", " + str(r[0]) + ", " + str(r[1]) + ", " + str(r[2]) + ", " + str(r[4]) + ", " + str(r[5]) + "\n"
            forStore_d[i].write(string)